...
public class BanqueV1Application {
    private static final Logger LOGGER = LoggerFactory.getLogger(BanqueV1Application.class);
	...
	public static void main(String[] args)  throws CompteException {
        ...
        // Le code à développer dans le cadre de ce TP doit permettre d'exécuter les cas d'usage ci-dessous.
		/*
        ...
        CompteCourant ccDominique = banqueService.creerCompteCourant(maBanque, dominique, "5678");
        ...
        CompteEpargne ceThibault = banqueService.creerCompteEpargne(maBanque, thibault, "1234", 3000.0, 0.02);
        ...
        try {
            ...
            LOGGER.error("*** Impossible de débiter le compteEpargne1: " + ce.getMessage());
        }
        LOGGER.info("*********************************************************\n");
        ...
        CarteBancaire cbDominique = banqueService.creerCarte(maBanque, dominique, ccDominique);
        LOGGER.info("*********************************************************\n");
        ...
        try
        {
            List<String> comptesPaulette = gabier1.accesComptes(maBanque, cbPaulette.getNumCarte(), cbPaulette.getCodePin());
            LOGGER.info("*** Liste de comptes de Paulette:");
            for(String rib: comptesPaulette) {
                LOGGER.info("***  - " + rib);
            }
            int numOperation = gabier1.retirerEspeces(maBanque, comptesPaulette.get(0), 314.0);
            LOGGER.info(String.format("*** Opération réalisée: %d  ***", numOperation));
            compteBancaireService.afficherSyntheseOperations(ccPaulette);
        }
        catch (SystemeBancaireException e)
        {
            LOGGER.error("Accès impossible aux comptes de Paulette: " + e.getMessage());
        }
        LOGGER.info("*********************************************************\n");
        ...
        try
        {
            List<String> comptesDominique = gabier1.accesComptes(maBanque, cbDominique.getNumCarte(), "CODE PIN FAUX");
            LOGGER.info("*** Liste de comptes de Dominique:");
            for(String rib: comptesDominique) {
                LOGGER.info("***  - " + rib);
            }
            int numOperation = gabier1.retirerEspeces(maBanque, comptesDominique.get(0), 567.0);
            LOGGER.info(String.format("*** Opération réalisée: %d  ***", numOperation));
        }
        catch (SystemeBancaireException e)
        {
            LOGGER.error("Accès impossible aux comptes de Dominique: " + e.getMessage());
            compteBancaireService.afficherSyntheseOperations(ccDominique);
        }
        LOGGER.info("*********************************************************");
        */
