...
@Component
public class ScenarioTest implements CommandLineRunner
{
    ...
    public void run(String... args) throws Exception
    {
        ...
        try
        {
            List<String> comptesPaulette = gabier.accesComptes(maBanque, cbPaulette.getNumCarte(), cbPaulette.getCodePin());
            LOGGER.info("*** Liste de comptes de Paulette:");
            for(String rib: comptesPaulette) {
                LOGGER.info("***  - " + rib);
            }
            int numOperation = gabier.retirerEspeces(maBanque, comptesPaulette.get(0), 314.0);
        ...
        try
        {
            List<String> comptesDominique = gabier.accesComptes(maBanque, cbDominique.getNumCarte(), "CODE PIN FAUX");
            LOGGER.info("*** Liste de comptes de Dominique:");
            for(String rib: comptesDominique) {
                LOGGER.info("***  - " + rib);
            }
            int numOperation = gabier.retirerEspeces(maBanque, comptesDominique.get(0), 567.0);
            LOGGER.info(String.format("*** Opération réalisée: %d  ***", numOperation));
        }
        catch (SystemeBancaireException e)
        {
